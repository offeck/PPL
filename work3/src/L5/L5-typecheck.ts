...
const checkEqualType = (te1: TExp, te2: TExp, exp: Exp): Result<true> =>
  equals(te1, te2)
    ? makeOk(true)
    : bind(unparseTExp(te1), (te1: string) => bind(unparseTExp(te2), (te2: string) => bind(unparse(exp), (exp: string) => makeFailure<true>(`Incompatible types: ${te1} and ${te2} in ${exp}`))));
...
export const L5typeof = (concreteExp: string): Result<string> => bind(p(concreteExp), (x) => bind(parseL5Exp(x), (e: Exp) => bind(typeofExp(e, makeEmptyTEnv()), unparseTExp)));
                            ...
                            : p.op === "pair?"
                              ...
                              : p.op === "symbol?"
                                ...
                                : p.op === "not"
                                  ...
                                  : p.op === "eq?"
                                    ? makeOk(makeProcTExp([T(), T()], makeBoolTExp()))
                                    : p.op === "string=?"
                                      ? makeOk(makeProcTExp([makeStrTExp(), makeStrTExp()], makeBoolTExp()))
                                      : p.op === "display"
                                        ? makeOk(makeProcTExp([T()], makeVoidTExp()))
                                        : p.op === "newline"
                                          ? makeOk(makeProcTExp([], makeVoidTExp()))
                                          : p.op === "cons"
                                            ? makeOk(((t: TExp) => makeProcTExp([t, makeListTExp(t)], makeListTExp(t)))(T()))
                                            : p.op === "car"
                                              ? makeOk(((t: TExp) => makeProcTExp([makeListTExp(t)], t))(T()))
                                              : p.op === "cdr"
                                                ? makeOk(((t: TExp) => makeProcTExp([makeListTExp(t)], makeListTExp(t)))(T()))
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
  ...
  const constraint2 = bind(thenTE, (thenTE: TExp) => bind(altTE, (altTE: TExp) => checkEqualType(thenTE, altTE, ifExp)));
  return bind(constraint1, (_c1: true) => bind(constraint2, (_c2: true) => thenTE));
};
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
  ...
  const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) => checkEqualType(body, proc.returnTE, proc));
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
  bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
    if (!isProcTExp(ratorTE)) {
      return bind(unparseTExp(ratorTE), (rator: string) => bind(unparse(app), (exp: string) => makeFailure<TExp>(`Application of non-procedure: ${rator} in ${exp}`)));
    }
    if (app.rands.length !== ratorTE.paramTEs.length) {
      return bind(unparse(app), (exp: string) => makeFailure<TExp>(`Wrong parameter numbers passed to proc: ${exp}`));
    }
    const constraints = zipWithResult((rand, trand) => bind(typeofExp(rand, tenv), (typeOfRand: TExp) => checkEqualType(typeOfRand, trand, app)), app.rands, ratorTE.paramTEs);
...
export const typeofLet = (exp: LetExp, tenv: TEnv): Result<TExp> => {
  ...
  const constraints = zipWithResult((varTE, val) => bind(typeofExp(val, tenv), (typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)), varTEs, vals);
...
export const typeofLetrec = (exp: LetrecExp, tenv: TEnv): Result<TExp> => {
  ...
  if (!allT(isProcExp, procs)) return makeFailure(`letrec - only support binding of procedures - ${format(exp)}`);
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> =>
  bind(
    typeofExp(exp.val, tenv), // 1. infer the value's actual type
    (valTE: TExp) =>
      bind(
        checkEqualType(valTE, exp.var.texp, exp), // 2. must equal the annotation
        (_) => makeOk(makeVoidTExp()),
      ),
  ); // 3. a define has type void
...
export const typeofProgram = (exp: Program, tenv: TEnv): Result<TExp> => {
  const typeofSeq = (exps: List<Exp>, tenv: TEnv): Result<TExp> => {
    if (!isNonEmptyList<Exp>(exps)) return makeFailure(`Program has no expressions`);
    const e = first(exps);
    const others = rest(exps);
    if (isEmpty(others)) return typeofExp(e, tenv); // last exp → program type
    const nextTEnv = isDefineExp(e)
      ? makeExtendTEnv([e.var.var], [e.var.texp], tenv) // a define extends the env for the rest...
      : tenv;
    return bind(typeofExp(e, tenv), (_) => typeofSeq(others, nextTEnv)); // ...but is still checked here
  };
  return typeofSeq(exp.exps, tenv);
};
