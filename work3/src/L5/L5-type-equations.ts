...
export const extendPool = (exp: A.Exp, pool: Pool): Pool => cons({ e: exp, te: T.makeFreshTVar() }, pool);
...
const extendPoolVarDecl = (vd: A.VarDecl, pool: Pool): Pool => cons({ e: A.makeVarRef(vd.var), te: vd.texp }, pool);

export const inPool = (pool: Pool, e: A.Exp): Opt.Optional<T.TExp> => {
  const exp = R.find(
    (item) => item.e === e || (A.isVarRef(item.e) && A.isVarRef(e) && item.e.var === e.var) || (A.isLitExp(item.e) && A.isLitExp(e) && item.e.val === e.val),
      ...
      : A.isProcExp(e)
        ...
        : A.isLitExp(e) && V.isEmptySExp(e.val)
          ...
          : A.isLitExp(e) && V.isCompoundSExp(e.val)
            ...
            : A.isCompoundExp(e)
              ? extendPool(e, reducePool(findVars, A.expComponents(e), pool))
              : makeEmptyPool();
...
export const poolToEquations = (pool: Pool): Opt.Optional<Equation[]> => {
  ...
  return Opt.mapv(
    Opt.mapOptional((e: A.Exp) => makeEquationsFromExp(e, pool), R.map(R.prop("e"), poolWithoutVars)),
...
export const makeEquationsFromExp = (exp: A.Exp, pool: Pool): Opt.Optional<Equation[]> =>
  ...
  // Type(Application) = Te
  A.isAppExp(exp)
    ? Opt.bind(inPool(pool, exp.rator), (rator: T.TExp) =>
        Opt.bind(
          Opt.mapOptional((e) => inPool(pool, e), exp.rands),
          (rands: T.TExp[]) => Opt.mapv(inPool(pool, exp), (e: T.TExp) => [makeEquation(rator, T.makeProcTExp(rands, e))]),
        ),
      )
    : // The type of procedure is (T1 * ... * Tn -> Te)
      ...
      ? Opt.bind(inPool(pool, exp), (left: T.TExp) =>
          Opt.mapv(
            Opt.bind(safeLast(exp.body), (last: A.CExp) => inPool(pool, last)),
            (ret: T.TExp) => [
              makeEquation(
                left,
                T.makeProcTExp(
                  R.map((vd) => vd.texp, exp.args),
                  ret,
                ),
              ),
            ],
        ...
        ? V.isEmptySExp(exp.val)
          ...
          : V.isCompoundSExp(exp.val)
            ...
            : isNumber(exp.val)
              ...
              : isBoolean(exp.val)
                ? Opt.mapv(inPool(pool, exp), (left: T.TExp) => [makeEquation(left, T.makeBoolTExp())])
                : isString(exp.val)
                  ? Opt.mapv(inPool(pool, exp), (left: T.TExp) => [makeEquation(left, T.makeStrTExp())])
                  : Opt.makeNone()
          ...
          : // The type of a boolean is Boolean
            ...
            : // The type of a string is String
              ...
              : // The type of a primitive procedure is given by the primitive.
                A.isPrimOp(exp)
                ? Opt.bind(inPool(pool, exp), (left: T.TExp) => Opt.mapv(Res.resultToOptional(TC.typeofPrim(exp)), (right: T.TExp) => [makeEquation(left, right)]))
...
export const inferType = (exp: A.Exp): Opt.Optional<T.TExp> => {
  ...
  return Opt.bind(sub, (sub: S.Sub) => Opt.mapv(texp, (texp: T.TExp) => S.applySub(sub, texp)));
};
...
export const solveEquations = (equations: Equation[]): Res.Result<S.Sub> => solve(equations, S.makeEmptySub());
...
const solve = (equations: Equation[], sub: S.Sub): Res.Result<S.Sub> => {
  ...
  const solveVarEq = (tvar: T.TVar, texp: T.TExp): Res.Result<S.Sub> => Res.bind(S.extendSub(sub, tvar, texp), (sub2) => solve(rest(equations), sub2));

  const bothSidesEqualVars = (eq: Equation): boolean => T.isTVar(eq.left) && T.isTVar(eq.right) && T.eqTVar(eq.left, eq.right);

  const handleBothSidesAtomic = (l: T.AtomicTExp, r: T.AtomicTExp): Res.Result<S.Sub> =>
    T.eqAtomicTExp(l, r) ? solve(rest(equations), sub) : Res.makeFailure(`Equation with non-equal atomic type ${format(eq)}`);

  const eq = makeEquation(S.applySub(sub, first(equations).left), S.applySub(sub, first(equations).right));

  return bothSidesEqualVars(eq)
    ? solve(rest(equations), sub)
    : T.isTVar(eq.left)
      ? solveVarEq(eq.left, eq.right)
      : T.isTVar(eq.right)
        ? solveVarEq(eq.right, eq.left)
        : T.isAtomicTExp(eq.left) && T.isAtomicTExp(eq.right)
          ? handleBothSidesAtomic(eq.left, eq.right)
...
const splitEquation = (eq: Equation): Equation[] =>
  T.isProcTExp(eq.left) && T.isProcTExp(eq.right)
    ? R.zipWith(makeEquation, cons(eq.left.returnTE, eq.left.paramTEs), cons(eq.right.returnTE, eq.right.paramTEs))
    : T.isListTExp(eq.left) && T.isListTExp(eq.right)
