...
// Work2
export const makeClassExp = (fields: VarDecl[], methods: Binding[]): ClassExp => ({ tag: "ClassExp", fields: fields, methods: methods });
...
export const parseL3 = (x: string): Result<Program> => bind(p(x), parseL3Program);

export const parseL3Program = (sexp: Sexp): Result<Program> =>
  sexp === "" || isEmpty(sexp)
    ? makeFailure("Unexpected empty program")
    : isToken(sexp)
      ? makeFailure(`Program cannot be a single token: ${format(sexp)}`)
      : isNonEmptyList<Sexp>(sexp)
        ? parseL3GoodProgram(first(sexp), rest(sexp))
        : makeFailure(`Unexpected type ${format(sexp)}`);

const parseL3GoodProgram = (keyword: Sexp, body: Sexp[]): Result<Program> =>
  keyword === "L3" && !isEmpty(body) ? mapv(mapResult(parseL3Exp, body), (exps: Exp[]) => makeProgram(exps)) : makeFailure(`Program must be of the form (L3 <exp>+): ${format([keyword, ...body])}`);

// Exp -> <DefineExp> | <Cexp>
export const parseL3Exp = (sexp: Sexp): Result<Exp> =>
  isCompoundSexp(sexp)
    ? isNonEmptyList<Sexp>(sexp)
      ? parseL3CompoundExp(first(sexp), rest(sexp))
...
export const parseL3CompoundExp = (op: Sexp, params: Sexp[]): Result<Exp> => (op === "define" ? parseDefine(params) : parseL3CompoundCExp(op, params));

// CompoundCExp -> IfExp | ProcExp | LetExp | LitExp | AppExp
export const parseL3CompoundCExp = (op: Sexp, params: Sexp[]): Result<CExp> => (isString(op) && isSpecialForm(op) ? parseL3SpecialForm(op, params) : parseAppExp(op, params));
    ...
    : op === "if"
      ...
      : op === "lambda"
        ? isNonEmptyList<Sexp>(params)
          ? parseProcExp(first(params), rest(params))
          : makeFailure(`Bad proc: ${params}`)
        : op === "let"
          ? isNonEmptyList<Sexp>(params)
            ? parseLetExp(first(params), rest(params))
          ...
          : op === "quote"
            ...
            : op === "class"
              ? params.length === 2
                ...
                : makeFailure(`class expects 2 arguments: ${format(params)}`)
              : makeFailure("Never");
...
export const parseDefine = (params: List<Sexp>): Result<DefineExp> =>
  isNonEmptyList<Sexp>(params)
    ? isEmpty(rest(params))
      ? makeFailure(`define missing 1 arguments: ${format(params)}`)
      : params.length > 2
        ? makeFailure(`define too many arguments: ${format(params)}`)
...
const parseGoodDefine = (variable: Sexp, val: Sexp): Result<DefineExp> =>
  !isIdentifier(variable) ? makeFailure(`First arg of define must be an identifier: ${format(variable)}`) : mapv(parseL3CExp(val), (value: CExp) => makeDefineExp(makeVarDecl(variable), value));

export const parseL3CExp = (sexp: Sexp): Result<CExp> =>
  isCompoundSexp(sexp)
    ? isNonEmptyList<Sexp>(sexp)
      ? parseL3CompoundCExp(first(sexp), rest(sexp))
...
const parseIfExp = (params: Sexp[]): Result<IfExp> =>
  params.length !== 3
    ? makeFailure(`Expression not of the form (if <cexp> <cexp> <cexp>): ${format(params)}`)
    : mapv(mapResult(parseL3CExp, params), (cexps: CExp[]) => makeIfExp(cexps[0], cexps[1], cexps[2]));
...
const parseLetExp = (bindings: Sexp, body: Sexp[]): Result<LetExp> => {
  ...
  return bind(bindingsResult, (bindings: Binding[]) => mapv(mapResult(parseL3CExp, body), (body: CExp[]) => makeLetExp(bindings, body)));
};
...
const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
  ...
  const vars = map((b: [string, Sexp]) => b[0], methods as [string, Sexp][]);
  const valsResult = mapResult(parseL3CExp, map(second, methods as [string, Sexp][]));
  return mapv(valsResult, (vals: CExp[]) =>
    makeClassExp(map(makeVarDecl, fields as string[]), zipWith(makeBinding, vars, vals)));
};

// sexps has the shape (quote <sexp>)
export const parseLitExp = (param: Sexp): Result<LitExp> => mapv(parseSExp(param), (sexp: SExpValue) => makeLitExp(sexp));

export const isDottedPair = (sexps: Sexp[]): boolean => sexps.length === 3 && sexps[1] === ".";

export const makeDottedPair = (sexps: Sexp[]): Result<SExpValue> => bind(parseSExp(sexps[0]), (val1: SExpValue) => mapv(parseSExp(sexps[2]), (val2: SExpValue) => makeCompoundSExp(val1, val2)));
...
export const parseSExp = (sexp: Sexp): Result<SExpValue> =>
  sexp === "#t"
    ...
    : sexp === "#f"
      ? makeOk(false)
      : isString(sexp) && isNumericString(sexp)
        ? makeOk(+sexp)
        : isSexpString(sexp)
          ? makeOk(sexp.toString())
          : isString(sexp)
            ? makeOk(makeSymbolSExp(sexp))
            : sexp.length === 0
              ...
              : isDottedPair(sexp)
                ...
                : isNonEmptyList<Sexp>(sexp)
                  ? // fail on (x . y z)
                    first(sexp) === "."
                    ? makeFailure(`Bad dotted sexp: ${format(sexp)}`)
                    : bind(parseSExp(first(sexp)), (val1: SExpValue) => mapv(parseSExp(rest(sexp)), (val2: SExpValue) => makeCompoundSExp(val1, val2)))
                  : makeFailure(`Bad sexp: ${sexp}`);
...
const unparseLitExp = (le: LitExp): string => (isEmptySExp(le.val) ? `'()` : isSymbolSExp(le.val) ? `'${valueToString(le.val)}` : isCompoundSExp(le.val) ? `'${valueToString(le.val)}` : `${le.val}`);

const unparseLExps = (les: Exp[]): string => map(unparseL3, les).join(" ");

const unparseProcExp = (pe: ProcExp): string => `(lambda (${map((p: VarDecl) => p.var, pe.args).join(" ")}) ${unparseLExps(pe.body)})`;

const unparseLetExp = (le: LetExp): string => `(let (${map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`, le.bindings).join(" ")}) ${unparseLExps(le.body)})`;
        ...
        : isLitExp(exp)
          ...
          : isVarRef(exp)
            ...
            : isProcExp(exp)
              ? unparseProcExp(exp)
              : isIfExp(exp)
                ? `(if ${unparseL3(exp.test)} ${unparseL3(exp.then)} ${unparseL3(exp.alt)})`
                : isAppExp(exp)
                  ? `(${unparseL3(exp.rator)} ${unparseLExps(exp.rands)})`
                  : isPrimOp(exp)
                    ? exp.op
                    : isLetExp(exp)
                      ? unparseLetExp(exp)
                      : isDefineExp(exp)
                        ...
                        : isClassExp(exp)
                          ? `(class (${map((f: VarDecl) => f.var, exp.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`, exp.methods).join(" ")}))`
                          : isProgram(exp)
