...
For the common case (text-heavy PDFs), use the converter script at `.Codex/skills/pdf-to-md/convert.py`.

### Step 3: Run the converter script

```bash
pip install pdfplumber python-bidi --break-system-packages -q
python3 .Codex/skills/pdf-to-md/convert.py input.pdf output.md
```
